...
-- Sautero shelf: Richard reviews contributions (select shared_with_sautero = true rows) and
...
alter table recipes add column if not exists shared_with_sautero boolean not null default false;
grant select (shared_with_sautero) on recipes to authenticated;
