-- Sautero — one-off fix (Richard, 16.7. večer): sautero@protonmail.com accepted the Add Company
...
update profiles set
  ...
  team_join_seen = true,
  in_team = true
where id = (select id from auth.users where email = 'sautero@protonmail.com');
...
where p.id = (select id from auth.users where email = 'sautero@protonmail.com');
